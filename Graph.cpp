...
void Graph::addPlaces(QString _name, QString _info)
{
    len++;
...
QString Graph::findMinTime(int _start, int _end) {
    std::vector<std::pair<uint32_t, uint32_t>> mintime = minTime(_start, _end);
    if (mintime.size() == 0) {
        err("minTime，目的地未接通");
        return QString("目的地未接通");
    }
    QString result = "minTime: ";
    result += QString::number(mintime[0].first);
    result += ",path: ";
    result += QString::number(mintime[0].second);
    for (int i = 1; i < mintime.size(); i++) {
        result += "-->";
        result += QString::number(mintime[i].second);
        result += "(";
        result += QString::number(mintime[i].first);
        result += "号线)";
    }
    //std::cout << "lowtime:" << result << '\n';
    return result;
}
